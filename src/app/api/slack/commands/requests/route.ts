...
export async function POST(request: NextRequest) {
  ...
  try {
    const user = await upsertSlackUser(workspace.id, slackUserId);

    const [{ rows: myRequests, totalCount }, waitingRequests] = await Promise.all([
      listRequestsByRequester(workspace.id, user.id),
      listRequestsWaitingForApprover(workspace.id, user.id),
    ]);
