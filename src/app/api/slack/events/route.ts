...
async function publishHomeView(timer: RequestTimer, slackTeamId: string, slackUserId: string): Promise<void> {
  ...
  const [{ rows: recentRequests, totalCount: myRequestsTotalCount }, waitingRequests, isAdmin, billingState] = await timer.time(
    "db",
    ...
    () =>
      Promise.all([
        ...
        // M10.2/M10.3: cheap even for a non-admin viewer — Home always needs
        // it to decide whether an admin sees Free or Pro billing copy and
        // which billing buttons to show, and this whole handler already runs
        // inside after() with no ack budget to protect, so there's no
        // correctness reason to gate it on isAdmin.
        getWorkspaceBillingState(workspace.id),
      ]),
  );
