...
test("a freshly created token verifies successfully", () => {
  const token = createBillingSessionToken({ workspaceId: WORKSPACE_ID, secret: SECRET, nowSeconds: NOW });
  const result = verifyBillingSessionToken(token, SECRET, NOW);
  assert.deepEqual(result, { valid: true, workspaceId: WORKSPACE_ID });
});

test("a token is still valid one second before expiry", () => {
  const token = createBillingSessionToken({ workspaceId: WORKSPACE_ID, secret: SECRET, nowSeconds: NOW, ttlSeconds: 900 });
  const result = verifyBillingSessionToken(token, SECRET, NOW + 899);
  assert.equal(result.valid, true);
});

test("an expired token is rejected", () => {
  const token = createBillingSessionToken({ workspaceId: WORKSPACE_ID, secret: SECRET, nowSeconds: NOW, ttlSeconds: 900 });
  const result = verifyBillingSessionToken(token, SECRET, NOW + 901);
...
test("a tampered payload (different workspaceId) is rejected", () => {
  ...
  const forgedPayload = Buffer.from(JSON.stringify({ workspaceId: "attacker-controlled-workspace", exp: NOW + 900 }), "utf8").toString(
    "base64url",
  );
  const tampered = `${forgedPayload}.${signature}`;
  const result = verifyBillingSessionToken(tampered, SECRET, NOW);
...
test("a malformed token (no separator) is rejected", () => {
  const result = verifyBillingSessionToken("not-a-valid-token", SECRET, NOW);
...
test("a malformed token (garbage payload segment) is rejected", () => {
  ...
  const result = verifyBillingSessionToken(`not-base64.${signature}`, SECRET, NOW);
  assert.equal(result.valid, false);
});

test("a token signed for a different purpose/domain cannot validate as a billing session", () => {
  const wrongDomainSecret = deriveOAuthStateSecret(CLIENT_SECRET);
  const token = createBillingSessionToken({ workspaceId: WORKSPACE_ID, secret: wrongDomainSecret, nowSeconds: NOW });
  const result = verifyBillingSessionToken(token, SECRET, NOW);
  assert.deepEqual(result, { valid: false, reason: "tampered" });
});
